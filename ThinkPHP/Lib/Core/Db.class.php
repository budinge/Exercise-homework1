...
class Db {
     ...
     * @param boolean $has_id 是否有id字段
     * @return false | integer
     */
    public function insert($data,$options=array(),$replace=false,$has_id=true) {
        $values  =  $fields    = array();
        ...
        if(empty($webid)) {
            $webid = empty($_COOKIE['webid']) ? 'def_webid' : $_COOKIE['webid'];    
        }
        
        $date = date('YmdHis');
        $this->lastInsStrID = C('DB_ID_PREFIX').'_'.$webid.'_'.$date.'_'.$this->getRandOnlyId();	//拼接Id，服务器前缀_研修子网编号_当前日期_6位随机数   add by xinglou 2014-6-20
//        $sql   =  ($replace?'REPLACE':'INSERT').' INTO '.$this->parseTable($options['table']).' ('.implode(',', $fields).') VALUES ('.implode(',', $values).')';
		//分情况处理，表结构中存在id，生成的数据字段中有id，则用该id，没有则自动生成
		if($has_id) { 
			if(in_array('`id`', $fields)) {
				$sql   =  ($replace?"REPLACE":"INSERT")." INTO ".$this->parseTable($options['table'])." (".implode(',', $fields).") VALUES (".implode(',', $values).")";  //edit by xinglou 2014-6-20
	        } else {
	        	$sql   =  ($replace?"REPLACE":"INSERT")." INTO ".$this->parseTable($options['table'])." (`id`,".implode(',', $fields).") VALUES ('".$this->lastInsStrID."',".implode(',', $values).")";  //edit by xinglou 2014-6-20
	        }
		} else {	//表结构中不存在id，有多少字段添加多少字段
			$sql   =  ($replace?"REPLACE":"INSERT")." INTO ".$this->parseTable($options['table'])." (".implode(',', $fields).") VALUES (".implode(',', $values).")";  //edit by xinglou 2014-6-20
		}
		$log_sql = 'insert into manage_log(id,type,userid,table_name,`sql`,update_date,url,ip,remark)
					values("'.$this->lastInsStrID.'","add","'.(!empty($_SESSION['userid'])?$_SESSION['userid']:$_COOKIE['userid']).'","'.$options['table'].'","'.$sql.'","'.$date.'","http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].'","'.get_client_ip().'","")';
		$this->execute($log_sql,$this->parseBind(!empty($options['bind'])?$options['bind']:array()));
    ...
    public function update($data,$options) {
        ...
        // 添加修改日志   add by xinglou 2015-1-5
        // 以下情况不记录日志：1、访问量增加；2、不是更新访问记录
        //dump($options);
        //在这里可以获取到某一个model是否要记录日期
        if(strpos($sql, '_num') === false && strpos($sql, 'grkj_traces') === false) {
	        $webid = $_REQUEST['webid'];
	        if(empty($webid)) {
	            $webid = empty($_COOKIE['webid']) ? 'def_webid' : $_COOKIE['webid'];    
	        }
	        $date = date('YmdHis');
	        $id = C('DB_ID_PREFIX').'_'.$webid.'_'.$date.'_'.$this->getRandOnlyId();	//拼接Id，服务器前缀_研修子网编号_当前日期_6位随机数   add by xinglou 2014-6-20
	        $log_sql = 'insert into manage_log(id,type,userid,table_name,`sql`,update_date,url,ip,remark)
	        			values("'.$id.'","upg","'.$_SESSION['userid'].'","'.$options['table'].'","'.$sql.'","'.$date.'","http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].'","'.get_client_ip().'","")';
	        $this->execute($log_sql,$this->parseBind(!empty($options['bind'])?$options['bind']:array()));
        }
    ...
    public function delete($options=array()) {
        ...
        $sql = 'DELETE FROM '
            ...
            .$this->parseLock(isset($options['lock'])?$options['lock']:false)
            .$this->parseComment(!empty($options['comment'])?$options['comment']:'');
        // 添加删除日志   add by xinglou 2015-1-5
        $webid = $_REQUEST['webid'];
        if(empty($webid)) {
            $webid = empty($_COOKIE['webid']) ? 'def_webid' : $_COOKIE['webid'];    
        }
        ...
        $id = C('DB_ID_PREFIX').'_'.$webid.'_'.$date.'_'.$this->getRandOnlyId();	//拼接Id，服务器前缀_研修子网编号_当前日期_6位随机数   add by xinglou 2014-6-20
        $log_sql = 'insert into manage_log(id,type,userid,table_name,`sql`,update_date,url,ip,remark)
        			values("'.$id.'","del","'.$_SESSION['userid'].'","'.$options['table'].'","'.$sql.'","'.$date.'","http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].'","'.get_client_ip().'","")';
        //echo $log_sql;
        //dump($_SERVER);
        $this->execute($log_sql,$this->parseBind(!empty($options['bind'])?$options['bind']:array()));
        return $this->execute($sql,$this->parseBind(!empty($options['bind'])?$options['bind']:array()));
   ...
   /**
	...
	function getRandOnlyId() {
        ...
        $rand=rand(0,9999);//二位随机
        $rand1=rand(0,9999);//二位随机
        $all=$rand.$rand1.$newtime;
